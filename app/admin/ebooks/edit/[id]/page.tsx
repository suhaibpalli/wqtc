...
export default function EditEBookPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchEBook = async () => {
    try {
      ...
      const ebook = data.result.find((e: any) => e.id === parseInt(id));
  ...
  const handleUploadCover = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData({ ...formData, coverImage: data.result.filename });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(`Failed to update ebook: ${error.msg || 'Unknown error'}`);
      }
    } catch (error) {
